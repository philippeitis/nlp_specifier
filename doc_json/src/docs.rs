...
impl RawSection {
    pub(crate) fn new(header: Option<String>) -> Self {
        RawSection {
            header,
            lines: Vec::new(),
        }
...
impl RawDocs {
    pub(crate) fn push_line<S: AsRef<str>>(&mut self, line: S) {
    ...
    pub(crate) fn push_lines(&mut self, lines: String) {
        lines.lines().for_each(|line| self.push_line(line));
    }

    pub(crate) fn consolidate(self) -> Docs {
        Docs::from(self)
    }
}
